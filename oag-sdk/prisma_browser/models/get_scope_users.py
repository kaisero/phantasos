...
class GetScopeUsers(BaseModel):
    """
    The users or user groups the rule applies to.
    """ # noqa: E501
    is_any: StrictBool = Field(description="Flag indicating if any users or user groups are included in the scope.", alias="isAny")
    users: List[UserRef] = Field(description="Users the rule applies to. [] = any user, [{id, name, email}] = specific users")
    user_groups: List[UserGroupRef] = Field(description="User groups the rule applies to. [] = any group, [{id, name}] = specific groups", alias="userGroups")
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of GetScopeUsers from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of GetScopeUsers from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "userGroups": [UserGroupRef.from_dict(_item) for _item in obj["userGroups"]] if obj.get("userGroups") is not None else None
        })
