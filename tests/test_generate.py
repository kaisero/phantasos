...
def test_generate_invokes_resolved_java(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("phantasos.generator.sdk.provision.resolve_java", lambda: Path("/fake/java"))
...
def test_generate_passes_discriminator_lookup_flag(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("phantasos.generator.sdk.provision.resolve_java", lambda: Path("/fake/java"))
    monkeypatch.setattr(generate, "ensure_jar", lambda: tmp_path / "oag.jar")
    captured: dict[str, list[str]] = {}
    ...
    generate.generate("spec.yaml", str(tmp_path), "pkg", oneof_discriminator_lookup=False)
    props = captured["cmd"][captured["cmd"].index("--additional-properties") + 1]
...
def test_generate_cmd_uses_template_dir() -> None:
    ...
    cmd = generate._oag_cmd("spec.yaml", "/out", "pkg", "urllib3", oneof_discriminator_lookup=True)
    assert "-t" in cmd
    i = cmd.index("-t")
    assert cmd[i + 1].endswith("oag_templates/python")


def test_generate_cmd_suppresses_metadata() -> None:
    from phantasos.generator.sdk import generate

    # generateMetadata=false stops OAG writing the `.openapi-generator/` folder.
    cmd = generate._oag_cmd("spec.yaml", "/out", "pkg", "urllib3", oneof_discriminator_lookup=True)
    gp = cmd[cmd.index("--global-property") + 1]
    assert "generateMetadata=false" in gp
...
def test_prune_removes_suppressed_files(tmp_path: Path) -> None:
    ...
    (tmp_path / ".github" / "workflows" / "python.yml").write_text("old", encoding="utf-8")
    (tmp_path / ".github" / "workflows" / "ci.yml").write_text("keep", encoding="utf-8")
    (tmp_path / "pkg").mkdir()
    (tmp_path / "pkg" / "__init__.py").write_text("", encoding="utf-8")
    # generator bookkeeping that must not ship with the SDK
    (tmp_path / ".openapi-generator-ignore").write_text("x", encoding="utf-8")
    (tmp_path / ".openapi-generator").mkdir()
    (tmp_path / ".openapi-generator" / "FILES").write_text("x", encoding="utf-8")
    ...
    # generator bookkeeping removed
    assert not (tmp_path / ".openapi-generator-ignore").exists()
    assert not (tmp_path / ".openapi-generator").exists()
...
def test_skip_validate_spec_flag_present_when_set(
    ...
    monkeypatch.setattr("phantasos.generator.sdk.provision.resolve_java", lambda: "java")
