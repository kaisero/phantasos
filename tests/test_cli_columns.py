...
from phantasos.generator.cli.ir import FlagKind


def _f(name: str, kind: FlagKind = "scalar", **kw: Any) -> FieldInfo:
...
def test_default_columns_prefers_known_names_then_scalars_capped() -> None:
    cols = default_columns(FIELDS)
    assert [c.path for c in cols] == [
        "id",
        "name",
        "status",  # preferred order first
        "created_at",
        "region",
        "zone",  # then declaration order, cap 6
    ]
    assert all(c.header == c.path for c in cols)


def test_default_columns_excludes_json_fields() -> None:
...
def test_resolve_columns_string_shorthand_and_entry() -> None:
...
def test_resolve_columns_rejects_bad_jmespath_syntax() -> None:
    with pytest.raises(ValueError, match="invalid JMESPath"):
        resolve_columns(["members[].]"], FIELDS, "device-group")


def test_resolve_columns_rejects_empty_expression() -> None:
...
def test_resolve_columns_rejects_unknown_root_field() -> None:
    with pytest.raises(ValueError, match="unknown field 'nope'"):
        resolve_columns(["nope.deeper"], FIELDS, "device-group")


def test_resolve_columns_skips_root_check_when_fields_unknown() -> None:
...
def test_resolve_columns_skips_root_check_for_non_field_roots() -> None:
