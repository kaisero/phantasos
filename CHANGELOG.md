...
- **prisma-access CLI (P1: all 12 sub-packages enrolled)** — the remaining ten federated sub-packages now ship in the CLI alongside P0's `objects`/`incidents`: `config_operations`, `config_setup`, `deployment_services`, `device_settings`, `identity_services`, `mobile_agent`, `network_services`, `posture`, `security_services`, and `ztna_connector`. Their 32 non-CRUD operations are mapped into the `request` namespace (e.g. `request config-setup snippet-snapshot publish`, `request security-services security-rule move`, `request identity-services certificate import`, `request ztna-connector connector start-packet-capture`), so the fail-loud federated build — which rejects any unmapped non-CRUD op — stays green across the whole product. `ztna_connector` joins `incidents` in X-PANW-Region's `required_for`, so its commands pre-flight the region (exit 2 naming `PANW_REGION` when unset, before any network call); the pure-CRUD subs run region-unset.
- **Connection flag vs. command-field collision fix** — a global connection flag (`--region`/`--tenant`, from `default_headers`) is now dropped from any single command whose own path/body/query field already renders that flag name, instead of declaring the parameter twice (a `SyntaxError` at import). The dropped flag's value still flows from its env var / active environment / config and the command-aware pre-flight still enforces it where required — only the redundant per-command flag is omitted; non-colliding connection flags are unaffected. Surfaced by prisma-access `remote_network`, whose `region` body field clashed with the X-PANW-Region connection flag.
