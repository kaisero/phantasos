...
class SdkConfig:
    ...
    auth: Optional[OAuthClientCredentials] = None
    pagination: Optional[CursorPagination] = None
    errors: Optional[NestedError] = None
    facade: Optional[Facade] = field(default_factory=Facade)
    # generic codegen patches (apostrophe / lenient enums / oneOf first-match) are on by default
    apply_generic_patches: bool = True

    def components(self):
        """Selected components in vendor order (skip None)."""
        return [c for c in (self.auth, self.pagination, self.errors, self.facade) if c is not None]
