...
**Architecture:** A `request:` mapping `{object, action}` becomes a `Command` with `verb="request"`, `object=<object>`, and the **action carried in the existing `variant` field** (with `variant_param=None`, so the oneOf-discriminator injection does NOT fire) — which makes the existing emitter/registration produce `request <object> <action>` with zero new command-tree machinery. Each request command has ONE binding (the SDK method), built from its path id (if any) + body model. `build_cli_ir` stops skipping `cfg.request` and emits these commands; the emitted app gains a `request` verb sub-app.
...
## Key design decisions (for the reviewer to scrutinize)
1. **Action carried in `Command.variant`** (not a new field). Rationale: the emitter's `typer_path`/`variant_groups` logic and `app.py` object-sub-Typer registration already key off `variant`, so `request <object> <action>` works with no new emission code. `variant_param` stays `None` for request commands, so the runtime's body-discriminator injection (`if cmd.variant and cmd.variant_param`) does NOT fire. Trade-off: "variant" now means "oneOf variant OR request action" — a mild semantic overload, documented at the field.
...
- Modify: `src/phantasos/generator/cli/ir.py` — `SubVerb` gains `"action"`.
- Modify: `src/phantasos/generator/cli/classify.py` — `_SUBVERB_PRIORITY["action"]`; `build_cli_ir` emits request commands.
...
def test_build_cli_ir_emits_request_commands():
    ...
    assert susp.verb == "request" and susp.object == "widget" and susp.variant == "suspend"
    assert susp.variant_param is None              # no discriminator injection for actions
...
- [ ] **Step 3: Add `"action"` to `SubVerb`** in `src/phantasos/generator/cli/ir.py`:
...
- [ ] **Step 3: Add `"request"` to `_VERBS`** in `src/phantasos/generator/cli/templates/_generated/app.py.jinja`:

```jinja
_VERBS = ["set", "del", "show", "request"]
```

(No other change: request commands have `variant=<action>`, so `variant_groups` includes `(request, <object>)`, the emitter sets `typer_path=[object, action]`, and `build_generated_app` creates the object sub-Typer under the `request` verb app and registers each action as a leaf — exactly like variant `set` commands.)
...
```bash
git add src/phantasos/generator/cli/templates/_generated/app.py.jinja tests/test_cli_emitted.py
git commit -m "feat(cli-gen): register the request verb sub-app; request <object> <action> dispatches"
```
